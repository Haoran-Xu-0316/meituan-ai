...
## Exchange interaction refinement

保持当前页面分布。未发送留言和未提交评价在页内更新或返回同一交换时保留在内存，模拟问答、改期和留言提交不会抹掉另一个表单的内容；成功提交的留言清空，结束后的表单仍关闭。邀请草稿继续使用本机持久保存，匹配入口显示继续填写邀请。筛选空态优先返回已有交换记录。

课程编号与文案区分等待回应、待改期、可记录成果和前序未完成。手机搜索与课程表单采用16px输入文字，常用小控件扩大触控区；保留横卡布局和照片比例，长内容可换行。系统减少动态会中断场景转场；液态选中背景在目标尺寸变化时重新落位，清理过时动画和监听。

改期错误与具体剩余课节对应，失败不改变已有约定。读取本机数据时逐条校验交换，损坏或重复记录单独排除并提示，其他记录和资料保留；旧版已完成课程仍兼容。
